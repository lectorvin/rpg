...
def Say(title, path): # if you want to say something to user, like advice from npc
    root = tkinter.Tk()
...
class Real_Object(sprite.Sprite):  # things, that will be in hero's inventarory
    ...
    for row in f:
        for col in row:
            ...
            if col == '&': # sword
                ...
                platforms.append(pf)
                entities.add(pf)
                if LevDown: # if we return from previous level, we spawn near door to next level
                    generateHero(x,y)
                ...
                if level != 1:
                    ...
                    platforms.append(pf)
                    entities.add(pf)
                if LevUp: # if we goint to next level, we spawn near door to previous level
                    generateHero(x, y)
...
while running:
    ...
    pygame.display.set_caption('Level '+str(level)+' life '+str(life)+' money '+str(money))
